...
void CPU_1d::computeBorder(func_ptr_t* mUserFuncs,
...
# pragma omp for
		for (int x = 0; x < haloSize; ++x) {
			...
			mUserFuncs[mCompFuncNumber[x]](result, source, time, x, 0, 0,
					parametrs, externalBorder);
		}
...
void CPU_1d::computeCenter(func_ptr_t* mUserFuncs,
		unsigned short int* mCompFuncNumber, double* result, double** source,
		double time, double* parametrs, double** externalBorder, int zCount,
		int yCount, int xCount, int haloSize) {
# pragma omp parallel
